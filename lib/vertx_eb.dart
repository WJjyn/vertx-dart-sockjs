...
class EventBus
{
     ...
     * @param headers Optional headers of the message
     */
    Future<VertxEventBusMessage> sendWithReplyAsync( String address, Object message, [Map<String, String> headers] )
    async
    {
        Completer<VertxEventBusMessage> out = new Completer( );

        _eb.send(
            address, message, _fromMapToObject( headers ), new JsFunction.withThis( new _ReplyConsumerCaller( ( VertxEventBusMessage msg )
        {
            out.complete( msg );
        } ) ) );
...
class VertxEventBusMessage<VALUE, REPLY>
{
    ...
    JsObject get headers
    => _headers;

    bool get expectReply
    => _wireObject.hasProperty( reply_function_property );
}
