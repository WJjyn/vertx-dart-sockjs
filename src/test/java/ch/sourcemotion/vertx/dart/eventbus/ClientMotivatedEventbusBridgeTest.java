...
public class ClientMotivatedEventbusBridgeTest extends AbstractClientServerTest {
  private static final Logger LOGGER = LoggerFactory.getLogger(
      ClientMotivatedEventbusBridgeTest.class);
  ...
  /**
   * Test event bus for events they are initiated from the client side.
   *
   * @param context
   * @throws Exception
   */
  ...
  public void clientMotivatedEventTest(TestContext context) throws Exception {
    ...
    vertx.eventBus().consumer("withReply", message ->
    {
      LOGGER.info("withReply -> Body: {} of type {}", message.body(),
          message.body() != null ? message.body().getClass().getName() : null);
    ...
    vertx.eventBus().consumer("doubleReply", message ->
    {
      ...
      {
        r1.result().reply(r1.result().body(),
            new DeliveryOptions().setHeaders(r1.result().headers()));
        async.countDown();
      });
    });
