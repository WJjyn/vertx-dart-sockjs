...
public abstract class AbstractClientServerTest {
  ...
  public void prepareClientsideTest(String testName) throws IOException {
    ...
    stdout = Paths.get(
        config.getProperty(WORK_DIR_CONFIG_KEY) + "/target/" + testName + "-stdout.txt");
    errout = Paths.get(
        config.getProperty(WORK_DIR_CONFIG_KEY) + "/target/" + testName + "-error.txt");
   ...
   * @throws Exception
   */
  public Future startTestClient(TestContext context, Async async, String testFile) throws
                                                                                   Exception {
    ...
    {
      try {
        final int processResult = new ProcessBuilder("pub", "run", "test", "-p", "phantomjs",
            testFile).directory(
            workDirPath.toFile()).redirectError(errout.toFile()).redirectOutput(
            stdout.toFile()).start().waitFor();
    ...
    new Thread(() ->
    {
      try {
        ...
        context.assertEquals(0, exitCode,
            "Test has failed on the client side. Exit code:" + exitCode);
   ...
   *
   * @return <code>true</code> when the client side test parse has succeeded. Otherwise
   * <code>false</code>
   * @throws IOException
   */
